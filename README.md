...
maintainer saw them: a comment, a label, or an assignment after the approval
re-freezes the approval to the current issue at dispatch, so a decision comment
or a retry after an agent stopped does not cancel the job, while a changed
title or body cancels it and asks for a fresh approval. The cancelled card says
which in words. Closing every blocker does not auto-start the dependent
