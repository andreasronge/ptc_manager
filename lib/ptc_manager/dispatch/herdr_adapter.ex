defmodule PtcManager.Dispatch.HerdrAdapter do
  ...
  defp review_instructions(_job, count) do
    ...
    Do not launch reviewers yourself. Before requesting review, run ALL repository-required validation, including the checks normally run by commit and pre-push hooks. Fix failures and regenerate artifacts before committing. Commit the clean, validated checkpoint, then run `$PTC_OPERATION_WRAPPER review --handoff-file /absolute/path/to/note.txt`. Write a short plain-text note outside the worktree explaining changes, validation and responses to earlier findings; no template is required. Requirements belong in the GitHub issue or its links, not the handoff. The reviewer continues its own session across rounds. PtcManager independently chooses and launches the reviewer and records the reviewed commit. Each completed assessment consumes one round; failed attempts do not. Failures still pause for a maintainer decision. A review passes when it reports no high or medium finding. Fix the high and medium findings of a changes_requested review, run relevant checks, commit, and request another round. Low findings returned with a passed review are advisory: do not edit for them, and record them in the pull request retrospective as untracked follow-up work. When the coordinator reports passed, publish that exact commit without further edits. A later failing check that requires changes returns you to validation, commit, and review; the earlier green assessment does not cover those changes. If the budget is zero, skip review.
    ...
    with %{agent_name: name, herdr_pane: old_pane} when is_binary(name) and is_binary(old_pane) <-
      ...
      continuation =
        if job.review_resume_mode == "publication" do
          ...
          "\n#{approval} Complete publication of that exact commit. Any low-severity finding below is advisory follow-up for the pull request retrospective, not work for this commit. Do not make discretionary changes or request another review of the same commit. If mandatory validation fails and requires edits, validate, commit, and request a new review before publication."
        else
